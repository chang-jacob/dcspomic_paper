...
# simulations were split into four jobs to improve processing time
...
n_inter <- 100 # number of samples simulated
n_intra <- 100 # number of "replicates" simulated

# Iterate through each "patient"
# Starting at two because there is no inter-patient variability with a single patient
pair <- "B_A"
results_ab <- results |> dplyr::filter(i_j == pair) # filter to a single cell type pair

vec <- 1:100
half <- length(vec) / 2
idxs <- as.vector(rbind(vec[1:half], vec[length(vec):(half + 1)]))

# As we iterate over sample size, to maximize the heterogeneity at each stage, we 
# are reordering the patients based on their variance

reordered_pats <- results_ab |>
  group_by(patient) |>
...
results_ab <- inner_join(results_ab, reordered_pats)
head(results_ab)
